...
analysisDir     = [indiviDir '/analysis'];
figureDir       = [analysisDir '/figures'];          
statsDir        = [analysisDir '/R'];      
...
%          ylabel('MVC (N)'); xlabel('digit');
    ...
    case 'STATS_OverallENS'
        ...
        subplot(1,2,1), myboxplot([D.group, D.hand], mean(D.ens,2),'style_tukey', 'plotall',0, 'xtickoff'), ylim([-7 -2]), ylabel('')
        subplot(1,2,2), myboxplot([D.group D.hand], mean(D.mmFull,2), 'style_tukey', 'plotall',0, 'xtickoff'),  ylim([-7 -2]), ylabel('')
        ...
        for i=1:length(D.SubjN)
            Di = getrow(D,i);
            Si.SubjN    = [i; i];
            Si.group    = [Di.group; Di.group];
            Si.ens      = [mean(Di.ens); mean(Di.mmFull)];
            Si.ensType  = [1; 2];
            S           = addstruct(S,Si);
        end;
...
%         MANOVA1(D2.group,D2.mmDim);              % mirroring
    ...
    case 'ENS_patternSimilarity'        % estimate similarity in patterns between controls and patients
        ...
        for hand=1:2
            ...
            for i = unique(Dc.SubjN)'
                ...
                Si.SubjN    = i;
                Si.r        = corr(isC.ens',mean(Dp.ens,1)');
                Si.group    = 2;
                Si.hand     = hand;
                S           = addstruct(S,Si);
            end;
    ...
    case 'ENS_dysIndivid'
        ...
        Dc = getrow(D,D.group==1);
        Dp = getrow(D,D.group==2);
        
        S = [];
        for i=1:length(Dp.SubjN)
            % patients
            Di  = getrow(Dp,i);
            dys = find(Di.dysFinger(6:10));
            nondys = find(~ismember(1:5,dys));
            x   = nonsym_squareform(Di.ens,'dim','red');
            x   = x + diag(nan(5,1));
            
            % when dystonic finger moves, what is enslaving in non-dystonic
            dys2nondys  = nanmean(nanmean(x(dys,nondys)));
            % when non-dystonic finger moves, what is enslaving in dystonic
            nondys2dys  = nanmean(nanmean(x(nondys,dys)));
            % when non-dystonic finger moves, what is enslaving in non-dystonic
            nondys2nondys  = nanmean(nanmean(x(nondys,nondys)));
            ...
            Si.grp      = 3;
            Si.ens      = nondys2nondys;
            S           = addstruct(S,Si);
        end;
        varargout = {S};
        
        save(fullfile(rootDir,'Individuation_EMG/analysis','ens_dysindivid.mat'),'-struct','S');
    ...
    case 'get_matchSubj'     % get subjects who have measurements for both cortical and enslaving patterns
        ...
        for sn=1:length(subj)
            for h=1:2
                Di = getrow(D,strcmp(D.subj,subj(sn)) & (D.hand+1)==h);
    ...
    case 'MODEL_corrBehImg'             % correlating behaviour (enslaving) and imaging
        ...
        for g=[1:2]
            Di = getrow(D,D.group==g);
            
            mEns = mean(Di.ens,1);
            mMM = mean(Di.mm,1);
            mDist = mean(Di.dist,1);
            
            rMean(g)    = corr(mEns',mDist');
            
            Di.ensMS    = bsxfun(@minus,Di.ens,mEns);
            Di.mmMS     = bsxfun(@minus,Di.mm,mMM);
            Di.distMS   = bsxfun(@minus,Di.dist,mDist);
            
            Di.r        = diag(corr(Di.ensMS',Di.distMS'));
    ...
    case 'FIG_TCS'
        D = load(fullfile(analysisDir,'ens_alldat_Peak.mat'));
        D = getrow(D,D.hand==2 & D.group==2);   % dystonia/left hand
    ...
    case 'FIG_patterns'                     % plot patterns for the different groups/hands
        ...
        h = plt.figure;
        for hl=1:4
            Di = getrow(D,D.handLabel==hl);
            x = nonsym_squareform(mean(Di.ens,1),'dim','red');
            
            plt.subplot(2,4,hl);
            plt.image(exp(x),'scale',sc,'leg','none');
            title(t{hl});
        end;
        plt.set(h,'ax','square');
