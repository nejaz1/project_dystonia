...
switch(what)
    ...
    case 'make_realign_unwarp'%_____STEP 4____________________________
        ...
        sn=varargin{1};
        spmj_realign_unwarp(baseDir, subj_name{sn}, run, 1, subj_NumVol(sn),'prefix',prefix,...
                            'use3D',use3D,'subfolderRawdata','') %76
    ...
    case 'make4d'
        ...
        for i=1:numel(run)
            clear fnames
            for j=1:subj_NumVol(sn)
                fnames{j}=fullfile(baseDir,'imaging_data_raw',subj_name{sn},[prefix,subj_name{sn},'_run',run{i},'_',num2str(j),'.nii']);
            end;
...
%         regions=[1 2 3    11 12 13];
        ...
        for s=sn
            ...
            for i=1:size(raw_data,1)
                ...
                [dir_name,name,ext,num]=spm_fileparts(raw_data(i,:));
